...
bun run lint                                     # oxlint
bun run build                                    # vite build -> dist/ask_bonk
...
bun run dev                                      # Vite/Flue local dev server
...
- `vite.config.ts` wires the Flue plugin before the Cloudflare plugin; production builds generate the Worker entrypoint and Flue Durable Object exports.
- `src/index.ts` is the direct-Wrangler test shim used by `wrangler.test.jsonc`; it exports the default app and `RepoAgent` only.
- `src/app.ts` owns the Hono app, explicitly mounts the Flue GitHub channel, handles auth endpoints, `/stats`, the legacy `/webhooks` route, and `handleGitHubDelivery()`.
- `src/channels/github.ts` exports the Flue GitHub channel mounted at `/channels/github`; its canonical webhook is `/channels/github/webhook`.
- `src/github-workflow-jobs.ts` owns the setup, track, and finalize job logic called by the OIDC-protected `/api/github/*` compatibility routes. Flue v2 removed its workflow abstraction.
...
- `ae_queries/` SQL files are imported with Vite's `?raw` query for `/stats` Analytics Engine endpoints.
...
- The `/api/github/setup`, `POST /api/github/track`, and `PUT /api/github/track` handlers must keep the OIDC checks above before calling their job functions.
...
- Reevaluate moving or reducing `RepoAgent` only when Bonk can run OpenCode inside a Flue-managed isolated sandbox and use durable Flue agent state as the primary execution record.
