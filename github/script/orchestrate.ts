// Consolidated pre-flight orchestrator for the Bonk GitHub Action.
//
// Replaces 6 separate bun invocations (permissions, setup, version, prompt,
// oidc-exchange, track) with a single process. This eliminates ~6 bun cold
// starts (~15-25s) and enables parallelism between independent steps.
//
// The control flow, OIDC validation, and output semantics are identical to
// the original multi-step action.yml. Finalize remains a separate step
// (runs with if: always()).
//
// Outputs (via GITHUB_OUTPUT):
//   skip          - "true" if workflow should skip remaining steps
//   version       - resolved opencode version for cache key
//   dev           - "true" if using dev branch
//   cacheable     - "true" if version is suitable for caching
//   is_fork       - "true" if PR is from a fork
//   value         - the prompt for OpenCode
//   oidc_failed   - "true" if OIDC exchange failed
//   run_opencode  - "true" if fork PR should still run OpenCode
...
// Returns true if the workflow should skip (setup created a PR).
async function checkSetup(): Promise<boolean> {
  ...
  if (!issueNumber) {
    if (
      ...
      return true; // unreachable — setFailed exits
    }
  ...
  try {
    ...
    // exchange step handle success/failure.
...
// ---------------------------------------------------------------------------
// Version resolution (from version.ts)
// ---------------------------------------------------------------------------
...
async function resolveVersion(): Promise<VersionResult> {
...
// ---------------------------------------------------------------------------
// Prompt building (from prompt.ts)
// ---------------------------------------------------------------------------
...
async function exchangeOidc(): Promise<OidcResult> {
  ...
  function failWithFallback(reason: string): OidcResult {
...
// Returns true if OpenCode should run for this fork PR.
async function handleFork(oidcFailed: boolean): Promise<boolean> {
...
async function main() {
  ...
  if (shouldSkip) {
    // setup.ts set skip=true or called setFailed (which exits).
    // If we reach here, skip was set via output — action should skip remaining steps.
    return;
  }
  ...
  // Set version outputs (consumed by cache step in action.yml)
  ...
  // Step 4: Fork handling (from action.yml "Handle fork PR" step)
  // Condition: is_fork == true
  if (promptResult.isFork) {
    const runOpencode = await handleFork(oidcResult.failed);
    core.setOutput("run_opencode", String(runOpencode));
    // Fork path complete — skip the non-fork OIDC enforcement and tracking
    return;
  }

  // Step 5: Require OIDC for non-fork runs
  // From action.yml "Require OIDC for non-fork runs" step
  if (oidcResult.failed) {
    core.setFailed("OIDC token exchange failed. Ensure id-token: write is configured.");
    return; // unreachable — setFailed exits
  }

  // Step 6: Track Bonk run (only for non-fork, OIDC-succeeded runs)
  // From action.yml: if: is_fork != true && oidc_failed != true
  await trackRun();
}
