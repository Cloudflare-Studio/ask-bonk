...
export async function validateGitHubOIDCToken(
	...
	return Result.tryPromise({
		...
		catch: (e) => new OIDCValidationError({ message: e instanceof Error ? e.message : 'Unknown error', cause: e }),
	});
}
...
async function generateInstallationToken(env: Env, installationId: number, options?: ScopedTokenOptions): Promise<string> {
	const auth = createAppAuth({
		appId: env.GITHUB_APP_ID,
		privateKey: env.GITHUB_APP_PRIVATE_KEY,
		installationId,
...
export async function handleGetInstallation(env: Env, owner: string, repo: string): Promise<GetInstallationResponse> {
...
export async function handleExchangeToken(
	...
	return Result.tryPromise({
		try: async () => {
			...
			return new GitHubAPIError({ operation: 'generateInstallationToken', cause: err });
		},
	});
}
...
export async function handleExchangeTokenForRepo(
	...
	if (!body.owner || !body.repo) {
		return Result.err(new AuthorizationError({ message: 'Missing owner or repo in request body', reason: 'invalid_format' }));
	}
	...
	return Result.tryPromise({
		try: async () => {
			...
			return new GitHubAPIError({ operation: 'generateCrossRepoToken', cause: err });
		},
	});
}
...
export async function handleExchangeTokenWithPAT(
	...
	if (env.ENABLE_PAT_EXCHANGE !== 'true') {
		return Result.err(new AuthorizationError({ message: 'PAT exchange is disabled', reason: 'invalid_token' }));
	}
	...
	if (!pat.startsWith('github_pat_') && !pat.startsWith('ghp_')) {
		return Result.err(new AuthorizationError({ message: 'Invalid token format - expected a GitHub PAT', reason: 'invalid_format' }));
	}
